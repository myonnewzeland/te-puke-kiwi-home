...
import { AlertTriangle, UtensilsCrossed, Bath, WashingMachine, Wifi, Car, Tv, TreePine, Droplets } from "lucide-react";
        ...
        <div className="container-narrow">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
            {accommodations.map((a, i) => (
              <div
                key={a.name}
                className="bg-card rounded-lg overflow-hidden border border-border shadow-[var(--shadow-card)] opacity-0 animate-scale-in hover:-translate-y-1 hover:shadow-[var(--shadow-elevated)] hover:border-primary/30 transition-all duration-300"
          ...
          <div ref={facilitiesRef} className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {facilities.map((f, i) => (
              <div
                key={f.label}
                data-reveal
                className="facility-card group flex flex-col items-center gap-2 bg-accent rounded-lg p-5 text-center border border-transparent cursor-default"
                style={{ "--i": i } as React.CSSProperties}
              >
                <f.icon className="h-7 w-7 text-primary transition-transform duration-300 group-hover:animate-[wiggle_0.4s_ease-in-out]" />
